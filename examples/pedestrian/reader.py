...
@File          :reader.py
@Description:  :
    ref: https://sourcegraph.com/github.com/vita-epfl/trajnetplusplustools/-/blob/trajnetplusplustools/reader.py
@Date          :2022/10/07 11:50:27
@Author        :aqiu
@version       :1.0
'''
# import os
...
class Reader(object):
    ...
    def __init__(self, input_file, scene_type=None, image_file=None):
        if scene_type is not None and scene_type not in {
                'rows', 'paths', 'tags'
        }:
    ...
    def paths_to_xy(paths):
        ...
        pedestrians = set(row.pedestrian for path in paths for row in path
